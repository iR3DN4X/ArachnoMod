 ...
 * (The wrapper carries min/max so commands can range-check, since ForgeConfigSpec doesn't expose
...
object Config {
    ...
    /**
     * Write the loaded config back to disk NOW. ForgeConfigSpec.ConfigValue.set() only updates the
     * IN-MEMORY value — without an explicit spec save, every /spider config change silently
     * reverts on restart (verified on the NeoForge sibling: file still had the old value after a
     * "(saved)" set).
     */
    fun saveNow() {
        SPEC.save()
    ...
    sealed class Entry(val path: String) {
        ...
        class D(path: String, private val value: ForgeConfigSpec.DoubleValue, val min: Double, val max: Double) : Entry(path) {
            override fun get() = value.get()
            fun set(v: Double) { value.set(v.coerceIn(min, max)); saveNow() }
        }
        class I(path: String, private val value: ForgeConfigSpec.IntValue, val min: Int, val max: Int) : Entry(path) {
            override fun get() = value.get()
            fun set(v: Int) { value.set(v.coerceIn(min, max)); saveNow() }
        }
        class B(path: String, private val value: ForgeConfigSpec.BooleanValue) : Entry(path) {
            override fun get() = value.get()
            fun set(v: Boolean) { value.set(v); saveNow() }
        }
        /** A sound-event resource location like "minecraft:block.moss.step". */
        class Sound(path: String, private val value: ForgeConfigSpec.ConfigValue<String>) : Entry(path) {
            override fun get() = value.get()
            fun set(v: String) { value.set(v); saveNow() }
        }
    }

    private fun define(path: String, default: Double, min: Double, max: Double, vararg comment: String): ForgeConfigSpec.DoubleValue {
        val v = BUILDER.comment(*comment).defineInRange(path, default, min, max)
        entries.add(Entry.D(path, v, min, max))
        return v
    }
    private fun define(path: String, default: Int, min: Int, max: Int, vararg comment: String): ForgeConfigSpec.IntValue {
        val v = BUILDER.comment(*comment).defineInRange(path, default, min, max)
        entries.add(Entry.I(path, v, min, max))
        return v
    }
    private fun define(path: String, default: Boolean, vararg comment: String): ForgeConfigSpec.BooleanValue {
        val v = BUILDER.comment(*comment).define(path, default)
        entries.add(Entry.B(path, v))
        return v
    }
    private fun defineSound(path: String, default: String, vararg comment: String): ForgeConfigSpec.ConfigValue<String> {
        val v = BUILDER.comment(*comment).define(path, default)
    ...
    val CONFIG_VERSION_VALUE: ForgeConfigSpec.IntValue = BUILDER
    ...
    val RELOCATE_DISTANCE = define("relocateDistanceBlocks", 192.0, 0.0, 4096.0,
        "If the living spider ends up this far from EVERY player while its chunk is STILL",
    ...
    val CHASE_PATHFINDING = define("chasePathfinding", true,
        "If true, a chasing spider STEERS around walls and cliffs it cannot climb instead of",
    ...
    val NETHERITE_ARMOR = define("netheriteArmor", 20.0, 0.0, 30.0,
        "The NETHERITE variant's armor points (default 20 = a full netherite suit; vanilla",
    ...
    val NETHERITE_ARMOR_TOUGHNESS = define("netheriteArmorToughness", 12.0, 0.0, 20.0,
        "The NETHERITE variant's armor toughness (default 12 = a full netherite suit).",
    ...
    val NETHERITE_KNOCKBACK_RESISTANCE = define("netheriteKnockbackResistance", 0.4, 0.0, 1.0,
        "The NETHERITE variant's knockback resistance, 0-1 (default 0.4 - a netherite suit",
        "is 0.1 per piece). Mostly cosmetic while the simulation owns the body's position,",
        "but it keeps the suit honest.")
    val NETHERITE_LEGS = define("netheriteLegs", 8, 1, 16,
        "How many legs the NETHERITE spider has (default 8 - the classic octopod). The",
    ...
    val POISON_SIZE = define("poisonSize", 1.0, 0.3, 5.0,
        "The POISON variant's FIXED size - it never grows or shrinks with distance (default",
    ...
    val HUNTER_BLINDNESS_RANGE = define("hunterBlindnessRange", 16.0, 0.0, 128.0,
        "How close (blocks) the stalking HUNTER has to be before it blinds you. While you stay",
    ...
    val POISON_EFFECT_SECONDS = define("poisonEffectSeconds", 30.0, 0.0, 3600.0,
        "How long (seconds) the Poison II from the poison variant's bite lasts.")
    ...
    val ENRAGED_MAX_HEALTH = define("enragedMaxHealth", 700.0, 1.0, 1000000.0,
        "Max health of an ENRAGED netherite spider (right-click the netherite variant with a",
        "netherite ingot to enrage it into a boss; it keeps its full armor suit).")
    val ENRAGED_SPEED_MULTIPLIER = define("enragedSpeedMultiplier", 1.5, 1.0, 8.0,
        "Chase-speed multiplier while enraged.")
    val ENRAGED_ATTACK_DAMAGE_HEARTS = define("enragedAttackDamageHearts", 10.0, 0.0, 100.0,
        "The enraged boss's bite damage in HEARTS. It always drops a FULL netherite block.")
