 ...
 *
 * **This file is VERSION-SPECIFIC (1.20.1).** The advancement API is one of the few that moved
 * between 1.20.1 and 1.21: here `getAdvancement(id)` returns a bare `Advancement`, whereas 1.21
 * renamed it to `get(id)` and wraps the result in an `AdvancementHolder`. The 1.21.1 projects
...
fun ServerPlayer.grantAdvancement(path: String) {
    ...
    val advancement = server.advancements
        .getAdvancement(ResourceLocation(SpiderAnimationMod.ID, path)) ?: return
...
/** Advancement ids — these must match the file names under `data/arachnomod/advancements/`. */
object Advancements {
